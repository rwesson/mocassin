...
!    call setContinuum()
    ...
    ! re-initialize opacities
!    grid3D%opacity = 0.

    ! calculate the opacities at every grid cell
!    do i = taskid+1, grid3D%nx, numtasks
!       do j = 1, grid3D%ny
!          do k = 1, grid3D%nz
!             call ionizationDriver(grid3D,i,j,k)
!          end do
!!       end do
!    end do
 
!    allocate(opacityTemp(0:nCells, nbins), stat = err)
!    if (err /= 0) then
!       print*, "! iterateMC: can't allocate array memory: opacityTemp "
!       stop
!    end if
...
!    call mpi_allreduce(grid3D%opacity, opacityTemp, size, &
...
!          do k = 1, grid3D%nz
!             do freq = 1, nbins
!                grid3D%opacity(grid3D%active(i,j,k),freq) = opacityTemp(grid3D%active(i,j,k),freq)
!             end do
!          end do
!       end do
!    end do
...
!***********************************************************************
    ...
    ! zero out PDF arrays
!    grid3D%recPDF     = 0.
!    if (lgDebug) grid3D%linePDF    = 0.
!    grid3D%totalLines = 0.  
           

    ! set the diffuse PDFs at every grid cell
!    do i = 1+taskid, grid3D%nx, numtasks
!       do j = 1, grid3D%ny
!          do k = 1, grid3D%nz
!             call emissionDriver(grid3D,i,j,k)
!          end do
!       end do
!    end do


 !   allocate(recPDFTemp(0:nCells, 1:nbins),&
 !               & stat = err)
 !   if (err /= 0) then
 ...
 !   if (lgDebug) then
 !      allocate(linePDFTemp(0:nCells, 0:nLines),&
 !           & stat = err)
 !      if (err /= 0) then
 !         print*, "! iterateMC: can't allocate array memory:&
 !              & opacityTemp "
...
!    allocate(totalLinesTemp(0:nCells), stat =&
!         & err)
!    if (err /= 0) then
!       print*, "! iterateMC: can't allocate array memory:&
!            & opacityTemp "
...
!    call mpi_allreduce(grid3D%recPDF, recPDFTemp, size, mpi_real&
!         &, mpi_sum, mpi_comm_world, ierr)
        
!    if (lgDebug) then
!       size =  (nCells+1)*nLines

!       call mpi_allreduce(grid3D%linePDF, linePDFTemp, size,&
!            & mpi_real, mpi_sum, mpi_comm_world, ierr)
!    end if

!    size = nCells+1

!    call mpi_allreduce(grid3D%totalLines, totalLinesTemp, size,&
!         & mpi_real, mpi_sum, mpi_comm_world, ierr)

!    do i = 1, grid3D%nx
!       do j = 1, grid3D%ny
!          do k = 1, grid3D%nz
!             grid3D%totalLines(grid3D%active(i,j,k)) = totalLinesTemp(grid3D%active(i,j,k))!
!             do freq = 1, nbins
!                grid3D%recPDF(grid3D%active(i,j,k),freq) = recPDFTemp(grid3D%active(i,j,k),freq)
!             end do
!             if (lgDebug) then
!                do freq = 1, nLines
!                   grid3D%linePDF(grid3D%active(i,j,k),freq) = linePDFTemp(grid3D%active(i,j,k),freq)
!                end do
!             end if
!          end do
!       end do
...
!    if ( associated(totalLinesTemp) ) deallocate(totalLinesTemp)
!    if (lgDebug) then
...
!***********************************************************************
    ...
    ! allocate and initialise plot%intensity array
!    allocate(plot%intensity(1:nGrids, 0:maxCells, 1:plot%nPlots), stat = err)
!    if (err /= 0) then
!       print*, "! mocassinPlot: can't allocate plot%ntensity array"
!       stop
!    end if
!    plot%intensity = 0.
